...
try {
  ...
  assert.equal(missing.ready_for_fitbit_api, false);
  assert.deepEqual(missing.missing_env, ['FITBIT_CLIENT_ID', 'FITBIT_CLIENT_SECRET', 'FITBIT_REDIRECT_URI']);
  assert.ok(missing.next_steps.some((step) => step.includes('FITBIT_CLIENT_ID')));
  ...
  assert.ok(doctorPayload.next_steps.some((step) => step.includes('FITBIT_CLIENT_ID')));
  ...
  assert.match(authWithoutEnv.stderr, /Missing required FITBIT environment variables/);
  ...
  const configPath = join(dir, '.fitbit-mcp', 'config.json');
