...
clearCache();
{
  ...
  await fetchWithCache('https://api.example/v1/q?afterDate=2026-05-01&beforeDate=2026-05-19&limit=10', { method: 'GET' }, opts);
  await fetchWithCache('https://api.example/v1/q?limit=10&beforeDate=2026-05-19&afterDate=2026-05-01', { method: 'GET' }, opts);
  await fetchWithCache('https://api.example/v1/q?beforeDate=2026-05-19&afterDate=2026-05-01&limit=10', { method: 'GET' }, opts);
...
process.env.FITBIT_NO_CACHE = 'true';
{
  let calls = 0;
  const inner = async () => { calls += 1; return okResponse(); };
  const opts = { innerFetch: inner, defaultTtlSeconds: 10, envVarBypass: 'FITBIT_NO_CACHE' };
  await fetchWithCache('https://api.example/v1/bypass', { method: 'GET' }, opts);
  await fetchWithCache('https://api.example/v1/bypass', { method: 'GET' }, opts);
  assert.equal(calls, 2, 'FITBIT_NO_CACHE=true must skip cache reads AND writes');
  assert.equal(getCacheStats().size, 0);
}
delete process.env.FITBIT_NO_CACHE;
