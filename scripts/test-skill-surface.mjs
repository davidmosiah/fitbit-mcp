...
assert.match(skill, /call fitbit_connection_status/);
...
const result = await run(["call", "fitbit_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "fitbit_connection_status", files: pkg.files }, null, 2));
