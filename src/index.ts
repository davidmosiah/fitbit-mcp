...
import { registerFitbitPrompts } from "./prompts/fitbit-prompts.js";
import { registerFitbitResources } from "./resources/fitbit-resources.js";
import { registerFitbitTools } from "./tools/fitbit-tools.js";
...
function createServer(): McpServer {
  ...
  registerFitbitTools(server);
  registerFitbitResources(server);
  registerFitbitPrompts(server);
  return server;
}
...
if (cliResult !== undefined) {
  ...
  const transport = process.env.FITBIT_MCP_TRANSPORT ?? (args.has("--http") ? "http" : "stdio");
