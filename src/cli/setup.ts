...
import { writeLocalConfig, type LocalFitbitConfig } from "../services/local-config.js";
...
export async function runSetupCommand(args: string[]): Promise<number> {
  ...
  const config: LocalFitbitConfig = {
    FITBIT_CLIENT_ID: options.clientId,
    FITBIT_CLIENT_SECRET: options.clientSecret,
    FITBIT_REDIRECT_URI: options.redirectUri,
    FITBIT_SCOPES: DEFAULT_SCOPES.join(" "),
    FITBIT_PRIVACY_MODE: options.privacyMode
  };
  if (options.cache) config.FITBIT_CACHE = options.cache;
  if (options.tokenPath) config.FITBIT_TOKEN_PATH = options.tokenPath;
  if (options.cachePath) config.FITBIT_CACHE_PATH = options.cachePath;
  ...
  if (options.json) console.log(JSON.stringify(setupOutput, null, 2));
    ...
    console.log("Fitbit MCP setup saved.");
    ...
    console.log("Secrets were saved only in the local Fitbit MCP config file.");
  }
...
function writeClientConfig(client: AgentClientName, homeDir: string): ClientConfigResult {
  ...
  const path = join(homeDir, ".fitbit-mcp", "mcp-configs", `${client}.json`);
...
function mergeClaudeConfig(homeDir: string): string {
  const path = process.platform === "darwin"
    ...
    : join(homeDir, ".fitbit-mcp", "mcp-configs", "claude-desktop.json");
  mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
  ...
  const next = {
    ...
    mcpServers: {
      ...mcpServers,
      fitbit: mcpConfigSnippet().mcpServers.fitbit
    }
  };
...
function writeHermesClientConfig(homeDir: string): ClientConfigResult {
  ...
  return {
    ...
    warnings: [
      "After editing Hermes MCP config, use `/reload-mcp` or `hermes mcp test fitbit`; do not restart the Hermes gateway for normal Fitbit data access.",
...
function mergeHermesConfig(configPath: string): string | undefined {
  ...
  if (/fitbit-mcp-unofficial|fitbit-mcp-server|fitbit-mcp/.test(existing) && /^\s*fitbit\s*:/m.test(existing)) {
    if (existing.includes(PINNED_NPM_PACKAGE)) return undefined;
    const backupPath = backupConfig(configPath);
    const updated = existing.replace(/fitbit-mcp-unofficial(?:@\d+\.\d+\.\d+)?/g, PINNED_NPM_PACKAGE);
  ...
  const backupPath = backupConfig(configPath);
  const next = existing.trimEnd().length ? addHermesFitbitBlock(existing) : snippet;
  writeFileSync(configPath, ensureReloadHint(next), { mode: 0o600 });
  chmodSync(configPath, 0o600);
  return backupPath;
}

function addHermesFitbitBlock(existing: string): string {
  const serverBlock = [
    "  fitbit:",
    "    command: npx",
    "    args:",
...
function backupConfig(path: string): string {
  const backupPath = `${path}.bak-fitbit-mcp-${new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "Z")}`;
...
function setupNextStep(client: AgentClientName, noAuth: boolean): string {
  const auth = noAuth ? "Run `fitbit-mcp-server auth`, then " : "";
  if (client === "hermes") {
    return `${auth}run \`fitbit-mcp-server doctor --client hermes\`, then use \`/reload-mcp\` or \`hermes mcp test fitbit\`.`;
  }
  return `${auth}run \`fitbit-mcp-server doctor\`.`;
}
