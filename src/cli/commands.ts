...
async function runOnboarding(args: string[]): Promise<number> {
  ...
  if (process.stderr.isTTY) {
    const lines = [
      "",
      `# Fitbit MCP — Wellness Onboarding (${flow.locale})`,
      "",
...
function printDoctor(status: Awaited<ReturnType<typeof buildConnectionStatus>>): void {
  ...
  if (status.client_checks?.hermes) {
    ...
    line(check(hermes.fitbit_server_configured), "configured");
...
function printHelp(): void {
  console.log(`Fitbit MCP Server

Usage:
  fitbit-mcp-server                 Start MCP stdio server
  fitbit-mcp-server --http          Start local HTTP MCP server
  fitbit-mcp-server setup           Guided setup, local config, and MCP client config
  fitbit-mcp-server doctor          Check setup and next steps
  fitbit-mcp-server doctor --json   Print setup status as JSON
  fitbit-mcp-server doctor --client hermes
  fitbit-mcp-server auth            Authorize Fitbit with local browser callback
  fitbit-mcp-server auth --no-open  Print auth URL without opening browser
  fitbit-mcp-server onboarding      Print the shared wellness onboarding flow (JSON to stdout)
  fitbit-mcp-server onboarding --locale pt-BR
