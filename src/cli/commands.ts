...
function printDoctor(status: Awaited<ReturnType<typeof buildConnectionStatus>>): void {
  ...
  console.log(`- Fitbit env vars: ${status.missing_env.length === 0 ? "ok" : `missing ${status.missing_env.join(", ")}`}`);
  ...
  if (status.client_checks?.hermes) {
    ...
    console.log(`  configured: ${hermes.fitbit_server_configured ? "ok" : "missing"}`);
