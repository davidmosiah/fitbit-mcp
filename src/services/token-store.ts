...
import type { FitbitTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<FitbitTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as FitbitTokenSet;
    } catch (error) {
  ...
  async write(tokens: FitbitTokenSet): Promise<void> {
