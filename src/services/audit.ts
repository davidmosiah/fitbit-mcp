...
export function buildPrivacyAudit(): Record<string, unknown> {
  const requiredEnv = ["FITBIT_CLIENT_ID", "FITBIT_CLIENT_SECRET", "FITBIT_REDIRECT_URI"];
  ...
  return {
    ...
    privacy_mode_default: parsePrivacyMode(value("FITBIT_PRIVACY_MODE")),
    raw_payloads_opt_in: true,
    gps_redaction_default: true,
    cache_enabled: parseBool(value("FITBIT_CACHE")),
    cache_path: value("FITBIT_CACHE_PATH") ?? join(homedir(), ".fitbit-mcp", "cache.sqlite"),
    token_path: value("FITBIT_TOKEN_PATH") ?? join(homedir(), ".fitbit-mcp", "tokens.json"),
    stdout_safe: true,
    secret_env_vars: ["FITBIT_CLIENT_SECRET"],
    ...
    notes: [
      "This is an unofficial Fitbit integration.",
      "OAuth tokens are stored locally and are not returned by tools.",
      "Raw Fitbit payloads require FITBIT_PRIVACY_MODE=raw or privacy_mode=raw.",
