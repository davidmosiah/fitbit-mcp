...
import type { PrivacyMode, FitbitTokenSet } from "../types.js";
...
export interface HermesClientCheck {
  ...
  fitbit_server_configured: boolean;
...
const REQUIRED_ENV = ["FITBIT_CLIENT_ID", "FITBIT_CLIENT_SECRET", "FITBIT_REDIRECT_URI"];
...
export async function buildConnectionStatus(options: ConnectionStatusOptions = {}): Promise<ConnectionStatus> {
  ...
  return {
    ok,
    ready_for_fitbit_api: ready,
    client: options.client,
    node: {
      version: process.versions.node,
      supported: nodeSupported
    },
    privacy_mode: parsePrivacyMode(value("FITBIT_PRIVACY_MODE")),
    ...
    oauth,
    cache: {
      enabled: parseBool(value("FITBIT_CACHE")),
      path: cachePath
    },
...
async function inspectToken(path: string, nowSeconds: number): Promise<ConnectionStatus["token"]> {
  try {
    ...
    const token = JSON.parse(text) as Partial<FitbitTokenSet>;
...
async function inspectHermesClient(homeDir: string): Promise<HermesClientCheck> {
  ...
  const skillPath = join(homeDir, ".hermes", "skills", "fitbit-mcp", "SKILL.md");
  const base: Omit<HermesClientCheck, "recommendations"> = {
    config_path: configPath,
    config_exists: false,
    fitbit_server_configured: false,
    package_pinned: false,
    skill_path: skillPath,
    skill_installed: false,
    direct_tool_prefix: "mcp_fitbit_",
  ...
  try {
    ...
    const check = {
      ...
      fitbit_server_configured: /fitbit-mcp-unofficial|fitbit-mcp-server|fitbit-mcp/.test(configText) && /^\s*fitbit\s*:/m.test(configText),
      package_pinned: /fitbit-mcp-unofficial@\d+\.\d+\.\d+/.test(configText),
...
function buildHermesRecommendations(check: Omit<HermesClientCheck, "recommendations">): string[] {
  ...
  if (!check.config_exists) {
    recommendations.push("Run `fitbit-mcp-server setup --client hermes --no-auth` to create a Hermes MCP config and local Hermes skill.");
  } else if (!check.fitbit_server_configured) {
    recommendations.push("Add a `fitbit` MCP server block to `~/.hermes/config.yaml`.");
  }
  if (check.config_exists && check.fitbit_server_configured && !check.package_pinned) {
  ...
  if (!check.skill_installed) {
    recommendations.push("Install the Hermes skill at `~/.hermes/skills/fitbit-mcp/SKILL.md` so agents prefer direct MCP tools over terminal workarounds.");
  }
  ...
  recommendations.push("After Hermes config changes, use `/reload-mcp` or `hermes mcp test fitbit`; do not run `hermes gateway restart` for normal Fitbit data access.");
...
function buildNextSteps(input: {
  ...
  if (!input.token.exists) {
    ...
    steps.push("Re-authorize with `fitbit-mcp-server auth`; the current token is expired and has no refresh token.");
  }
  if (input.oauth.scope_status === "missing_recommended") {
    steps.push(`Re-authorize with \`fitbit-mcp-server auth\` to grant recommended scopes: ${input.oauth.missing_recommended_scopes.join(" ")}.`);
  } else if (input.oauth.scope_status === "unknown" && input.token.exists && input.token.readable) {
    steps.push("Token scope is not recorded locally. If activity or stream tools fail with permission errors, re-authorize with `fitbit-mcp-server auth`.");
  }
  if (steps.length === 0) steps.push("Ready. Add this MCP server to your agent and start with fitbit_daily_summary.");
  return steps;
}
