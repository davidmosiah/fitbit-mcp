...
export type LocalFitbitConfig = Partial<Record<LocalConfigKey, string>>;

export interface LocalConfigReadResult {
  path: string;
  exists: boolean;
  values: LocalFitbitConfig;
  permissions?: string;
...
export function getLocalConfigPath(homeDir = homedir()): string {
  return join(homeDir, ".fitbit-mcp", "config.json");
}
...
export function writeLocalConfig(values: LocalFitbitConfig, homeDir = homedir()): string {
  const path = getLocalConfigPath(homeDir);
...
export function loadConfigSources(env: Record<string, string | undefined> = process.env, homeDir = homedir()): EffectiveConfigSources {
  ...
  const values: LocalFitbitConfig = {};
  let envUsed = false;
  let localUsed = false;
